...
import { isDownloadSig, isSubmittedSig, messageSig, isRequestSubmitted, userFoundSig, isApprovalSig, toastIdSignal, reset_signals } from "./PrasSignals";
import { effect, signal } from "@preact/signals-react";
import { ProgressToast } from "../components/ProgressToast";
import { isIOConnectedSig, socketEmit, socketSig } from "./realtime/socketIOSignals";
...
export function ProgressBar() {
    ...
    let socketSignal = isIOConnectedSig.value;
    toastIdSignal.value = toastIdRef.current;
    ...
    useEffect(() => {
        ...
        const handleMessage = (event: MessageEvent) => {
            try {
                ...
                if (data.event === "START_TOAST") {
                    toastIdSignal.value = toast.loading(
                ...
                if (data.event === "PROGRESS_UPDATE" && (isDownloadSig.value || isRequestSubmitted.value || isApprovalSig.value) && percent != null) {
                    ...
                    if (toastIdSignal.value === null) {
                        ...
                        toast.update(toastIdSignal.value, {
        ...
        socketSig.value?.on('message', handleMessage);
        return () => { socketSig.value?.off('message', handleMessage); };
    }, [socketSig, dispatch]);
