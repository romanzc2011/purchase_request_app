from loguru import logger
from sqlalchemy.orm import Session
import api.services.db_service as dbas

class UUIDService:
    """
    Service for UUID operations.
    This class can be instantiated and used by other modules.
    """
    
    def __init__(self):
        self._uuid_cache = {}
        logger.info("UUIDService initialized")
    
    def get_uuid_by_id(self, db_session: Session, ID: str):
        """
        Get UUID by ID with caching for better performance.
        """
        # Check cache first
        if ID in self._uuid_cache:
            logger.info(f"UUID cache hit for ID: {ID}")
            return self._uuid_cache[ID]
        
        # If not in cache, get from database
        try:
            # First try to find by sequential ID
            result = db_session.query(dbas.Approval).filter(dbas.Approval.ID == ID).first()
            if result:
                uuid = result.UUID
                # Store in cache
                self._uuid_cache[ID] = uuid
                logger.info(f"Added UUID to cache for ID: {ID}")
                return uuid
            else:
                logger.warning(f"No UUID found for ID: {ID}")
                return None
        except Exception as e:
            logger.error(f"Error getting UUID for ID {ID}: {e}")
            return None
    
    def get_uuids_by_ids(self, db_session: Session, ids: list):
        """
        Get UUIDs for multiple IDs.
        Returns a dictionary mapping IDs to UUIDs.
        """
        logger.info(f"Getting UUIDs for {len(ids)} IDs")
        
        result = {}
        for id in ids:
            uuid = self.get_uuid_by_id(db_session, id)
            if uuid:
                result[id] = uuid
        
        return result
    
    def clear_cache(self):
        """
        Clear the UUID cache.
        """
        self._uuid_cache = {}
        logger.info("UUID cache cleared")

# Create a singleton instance
uuid_service = UUIDService() 